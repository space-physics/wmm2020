...
def wmm(glats: np.ndarray, glons: np.ndarray, alt_km: float, yeardec: float, wmm_filename: str = None) -> xarray.Dataset:
    """
    ...
    for i, (glat, glon) in enumerate(zip(glats.ravel(), glons.ravel())):
        x, y, z, T, D, mI = wmm_ext.wmmsub(glat, glon, alt_km, yeardec, wmm_filename)
...
def transect(glats: np.ndarray, glons: np.ndarray, alt_km: np.ndarray, yeardec: np.ndarray, wmm_filename: str = None) -> dict:
    """
    ...
    for i, (_lat, _lon, _alt, _year) in enumerate(zip(*[v.ravel() for v in ref_input.values()])):
        x, y, z, T, D, mI = wmm_ext.wmmsub(_lat, _lon, _alt, _year, wmm_filename)
...
def wmm_point(glat: float, glon: float, alt_km: float, yeardec: float, wmm_filename: str = None) -> dict[str, float]:
    """
